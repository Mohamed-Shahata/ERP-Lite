...
interface PurchaseOrderItemsFormProps {
  ...
  onChange: (items: PurchaseOrderItemPayload[]) => void;
}
...
export function PurchaseOrderItemsForm({
  ...
  items,
  onChange,
}: PurchaseOrderItemsFormProps) {
  const { t } = useTranslations();

  function updateItem(index: number, patch: Partial<PurchaseOrderItemPayload>) {
    onChange(
  ...
  function addRow() {
    ...
    onChange([
      ...items,
      {
        ...
        quantity: 1,
        unitCost: firstProduct ? Number(firstProduct.costPrice) : 0,
      },
    ]);
  }
