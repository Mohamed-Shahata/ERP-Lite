...
import { listSalesReportRequest } from "@/lib/api/reports.api";
...
export default function SalesReportPage() {
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: () => listCustomersRequest({ limit: 100 }).then((res) => res.data),
  });
  ...
  const {
    ...
    queryKey: ["reports", "sales", range.from, range.to, customerId, status],
    queryFn: () =>
      listSalesReportRequest({
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <p className="text-xs text-slate-400 dark:text-slate-500">
        ...
        <span className="text-slate-600 dark:text-slate-300">
          {t("reports.categories.sales.title")}
        </span>
      </p>
      ...
      <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {t("reports.categories.sales.description")}
          </p>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <div className="flex items-center justify-end border-b border-slate-200 dark:border-slate-800 px-5 py-4">
          <ExportButtons
            ...
            onExportExcel={() =>
              exportRowsToExcel("sales-report", exportColumns, invoices)
            }
          />
        </div>
