...
export default function CategoriesPage() {
  ...
  const { data: categoriesResult, isLoading, error: loadError } = useQuery({
    queryKey: categoriesQueryKey,
    queryFn: () =>
      listCategoriesRequest({ page: categoriesPage, limit: categoriesPageSize }),
  });

  const categories = categoriesResult?.data ?? [];
  const totalCategories = categoriesResult?.meta.total ?? 0;
  const displayError =
    error ?? (loadError ? t("categories.loadError") : null);
